...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗧𝞝𝗣 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/xLxLxLrr) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗧𝞝𝗣─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
AliTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝞝𝗣 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/xLxLxLrr) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 1260465030 or user.id == 1260465030 or user.id == 1260465030:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا علش  الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def cmd(Alilll):
    await edit_or_reply(Alilll, AliTHS_cmd)
