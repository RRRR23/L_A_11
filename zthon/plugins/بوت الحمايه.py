# Zed-Thon - ZelZal
# Copyright (C) 2022 xLxLxLrr . All Rights Reserved
#
...
حقـوق : @xLxLxLrr
@L_A_1 - كتـابـة الملـف :  علش  الهيبــه"""
...
ANTI_DDDD_xLxLxLrr_MODE = ChatBannedRights(
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط على سـورس رنثـون",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت رنثـون",
    ...
    if is_locked(zed_id, "game") and event.message.media:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in zed_dev:
	        ...
	        try:
	            ...
	            await event.client(
	                EditBannedRequest(
	                    event.chat_id, event.sender_id, ANTI_DDDD_xLxLxLrr_MODE
	                )
	            )
	        except Exception as e:
	            await event.reply(
...
async def check_edit_media(event):
    if not event.is_group:
        return
    if event.is_group: #Write Code By T.me/L_A_1
        chat = await event.get_chat()
        admin = chat.admin_rights
        creator = chat.creator
        if not admin and not creator:
            return
    zed_dev = (925972505, 1895219306, 5280339206)  #Write Code By T.me/L_A_1
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
    user = await event.get_sender()
    if is_locked(zed_id, "document") and event.message.media: #Write Code By T.me/L_A_1
        if zelzal == malath or zelzal in zed_dev:
            return
        else:
	        try:
	            await event.delete() #Write Code By T.me/L_A_1
	        ...
	        except Exception:  #Write Code By T.me/L_A_1
	            update_lock(zed_id, "document", False)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝗥𝗨𝗡𝗧𝗛𝗢𝗡\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝗥𝗨𝗡𝗧𝗛𝗢𝗡\
